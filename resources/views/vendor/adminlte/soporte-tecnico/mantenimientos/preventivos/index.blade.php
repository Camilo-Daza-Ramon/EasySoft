...
@section('contentheader_title')
    <h1><i class="fa fa-calendar-check-o"></i>  Mantenimientos Preventivos</h1>
@endsection
...
@section('main-content')
            ...
            <div class="col-md-12">
                <div class="box box-info">
                    <div class="box-header bg-blue with-border">
                        <form id="form-buscar" action="{{route('preventivos.index')}}" role="search" method="GET">  
                            ...
                            <div class="btn-group pull-right">
                                ...
                                <ul class="dropdown-menu" role="menu">
                                    @permission('mantenimientos-preventivos-crear')
                                        <li><a href="#" data-toggle="modal" data-target="#mantenimientoAdd"><i class="fa fa-plus"></i> Agregar</a></li>
                                    @endpermission

                                    @permission('mantenimientos-preventivos-exportar')
                                ...
                                <div class="col-md-10 col-sm-11 col-xs-10">
                                    <div class="row">
                                        ...
                                        <div class="form-group col-md-3 col-sm-6">
                                            <input type="text" class="form-control" name="mantenimiento" placeholder="Número Mantenimiento" value="{{(isset($_GET['mantenimiento'])? $_GET['mantenimiento']:'')}}" autocomplete="off">
                                        </div>
                        ...
                        <table  id="areas" class="table table-bordered table-striped dataTable">
                            <tbody>
                                ...
                                @if($mantenimientos->count() > 0)
                                    ...
                                    <?php 
                                        $contador = date_diff(date_create($dato->fecha_programada), date_create($dato->fecha_programada));
                                    ...
                                    <tr>
                                        ...
                                        <th>
                                            <a href="{{route('preventivos.show', $dato->ProgMantid)}}">{{$dato->NumeroDeMantenimiento}}</a>
                                        </th>

                                        <td>
                                            {{$dato->tipo_mantenimiento->Descripcion}}
                                        </td>                                        
                                        <td>{{$dato->municipio->NombreMunicipio}}</td>
                                        <td>{{$dato->municipio->departamento->NombreDelDepartamento}}</td>
                                        <td>
                                            {{date('Y-m-d', strtotime($dato->fecha_programada))}}
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            @if(!empty($dato->fecha_cierre_hora_fin))
                                                <?php 
                                                    $contador = date_diff(date_create($dato->fecha_programada), date_create($dato->fecha_cierre_hora_fin));                                                    
                                                ?>                      
                                                {{($contador->invert)? 0 : $contador->format('%a')}} Días sin solución
                                            @else
                                                0 Días sin solución
                                            @endif
                                        </td>
                                        ...
                                        <td>
                                            @if($dato->estado != 'CERRADO' && $dato->fecha_programada <= date('Y-m-d'))
                                                @permission('mantenimientos-preventivos-cerrar')
                                                <a href="{{route('preventivos.cerrar_vista', $dato->ProgMantid)}}" class="btn btn-xs bg-purple"><i class="fa fa-calendar-check-o"></i></a>
                                                @endpermission
                                            @endif

                                            @permission('mantenimientos-preventivos-editar')
                                            <a href="{{route('preventivos.edit', $dato->ProgMantid)}}" class="btn btn-xs btn-primary"><i class="fa fa-edit"></i></a>
                                            @endpermission


                                            @permission('mantenimientos-preventivos-eliminar')
                                            <form action="{{route('preventivos.destroy', $dato->ProgMantid)}}" method="post" style="display: inline;">
                                            ...
                                            @if($dato->estado == 'CERRADO')
                                                ...
                                                <a target="_blank" href="{{route('preventivos.acta', ['id' => $dato->ProgMantid])}}" class="btn btn-xs btn-default" title="Generar Acta de Mantenimiento"><i class="fa fa-file-pdf-o"></i></a>
                                                @endpermission
                                            @endif
                                        </td>
    ...
    @permission('mantenimientos-preventivos-crear')
        @include('adminlte::soporte-tecnico.mantenimientos.preventivos.create')
    @endpermission
