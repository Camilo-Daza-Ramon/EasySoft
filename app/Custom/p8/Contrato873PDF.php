...
class Contrato873PDF extends Fpdf
{
	public $datos;
	...
	function pagina1(){
	    ...
	    $this->SetTextColor(27,27,27);
	    $this->SetFont('calibri','',9);
	    $this->MultiCell(90,4, utf8_decode("El servicio de Internet obedece a la implementación del programa INCENTIVOS A LA DEMANDA FASE II adelantado y financiado por el Fondo Único de TIC.\n1. La comercialización del servicio deberá dirigirse a hogares de estratos 1 y 2, beneficiarios inscritos en SISBEN, y podrán también incluirse los beneficiarios de la ley 1699 de 2013.\n2. El usuario que se beneficie del proyecto debe suministrar declaración juramentada de que es un nuevo usuario, es decir que él y los miembros de su núcleo familiar que residen en el mismo predio para el que se requiere la conexión, no han contado con la prestación del servicio de Internet fijo, al menos durante los seis meses anteriores a la suscripción.\n3. Las tarifas sociales establecidas para el pago mensual del servicio de Internet, según las condiciones técnicas establecidas en el presente documento, corresponden a $8.613 pesos para estrato 1 y $19.074 pesos para estrato 2.\n\n"),'L R', 'J', false);


	    foreach ($data['servicios'] as $servicio) {

	    	$this->Cell(20,4,"Tipo Servicio: ","L",0);
		    $this->Cell(60,4,utf8_decode($servicio->tipo_servicio),"B",0);
		    $this->Cell(10,4," ","R",0);
		    $this->Ln();

		    $this->Cell(20,4,"ID Servicio: ","L",0);
		    $this->Cell(60,4,$servicio->id,"B",0);
		    $this->Cell(10,4," ","R",0);
		    $this->Ln();

		    $this->Cell(20,4,"Plan: ","L",0);
		    $this->Cell(60,4,utf8_decode($servicio->descripcion),"B",0);
		    $this->Cell(10,4," ","R",0);	    
		    $this->Ln();

		    $this->Cell(20,4,"Valor: ","L",0);
		    $this->Cell(60,4,'$'.number_format($servicio->valor, 0,',','.'),"B",0);
		    $this->Cell(10,4," ","R",0);
		    $this->Ln();
		    $this->Cell(90,6," ","L R",0);
		    $this->Ln();
		}
	...
	function pagina2(){
	    ...
	    $this->SetTextColor(27,27,27);
	    $this->SetFont('calibri','',9);
	    $this->MultiCell(90,4, utf8_decode("1. EQUIPOS. En el evento que la empresa para efectos de la prestación del servicio, ponga a disposición del cliente equipos, éste último reconoce que la empresa es la propietaria legítima de los equipos asociados a la prestación de los servicios, dichos equipos son entregados en comodato y/o alquiler, para la prestación y operación del servicio, las partes acuerdan que deben ser entregados a la terminación del contrato, y que éstos serán facturados y debidamente cancelados por parte del cliente en caso de daño, pérdida, hurto, incluyendo daños derivados por variaciones de energía, descargas atmosféricas o el mal uso de los equipos, conforme el valor comercial de los mismos.\n2.CARGOS ADICIONALES: Las partes acuerdan que podrán incluirse otros cargos derivados de la prestación del servicio como; Cargos Eventuales: 1. Cargo por cada solicitud de traslado de equipos a una nueva dirección según la tarifa vigente. Costo de traslado: $60.000 + IVA. 2. Visitas Técnicas $15.000 + IVA, Cuando se presenten fallas en el servicio por causas imputables al cliente, bien sea por conexiones inadecuadas, mal uso o desconfiguración de los equipos, problemas técnicos originados por la red interna del cliente, virus o cualquier otra, que sea de su responsabilidad, el cliente autoriza a través del presente documento cobrar el valor correspondiente en la siguiente factura. 3. Excedente por metro de cable adicional al autorizado para la instalación $1.800 + IVA por metro, al momento de realizar traslados solicitados por usted. \n3. VELOCIDAD: No podemos garantizar vía wifi, la velocidad contratada, toda vez que ésta depende de múltiples aspectos, que no son siempre directamente imputables al proveedor del servicio, por ejemplo: equipos, configuración, tarjetas de red, obstáculos físicos permanentes y/o transitorios, entre otros. En caso de que un cliente perciba que la velocidad no cumple sus expectativas, deberá comunicarse con las líneas 018000945080 - 018000942422 - (607)6335080"),1, 'J', false);
	    $this->Ln();
	...
	function inf4(){
        ...
        $this->MultiCell(0,5,utf8_decode('1. Que el proyecto de incentivos a la demanda Fase II es financiado por el Fondo Único de Tecnologías de la Información y las Comunicaciones.'),0,'J',false);
	...
	function firmas(){
        ...
        //Firma Beneficiario
		if(!empty($data['firma'])){
			$this->Image('storage/'.$data['firma'], 30,null, 85, 0,'JPG');
		}
        
        $this->Ln();

        $this->Cell(20,5,'Firma:',0,0);
        $this->Cell(60,5,'','B',0,'L');
        $this->Ln();

        $this->Cell(20,5,'Nombre:',0,0);
        $this->Cell(60,5,utf8_decode($data['nombre_suscriptor']),'B',0,'L');
        $this->Ln();
        $this->Cell(20,5,utf8_decode('Cédula N°'),0,0);
        $this->Cell(40,5,$data['identificacion'],'B',0,'L');

        $this->Cell(20,5,utf8_decode('Teléfono N° '),0,0);
        $this->Cell(20,5,$data['telefono'],'B',0,'L');
        $this->Ln();

        $this->Cell(20,5,utf8_decode('Dirección'),0,0);
        $this->Cell(60,5,utf8_decode($data['direccion'] . ' ' . utf8_decode($data['barrio'])),'B',0,'L');
        $this->Ln();

        $this->Cell(20,5,utf8_decode('Municipio '),0,0);
        $this->Cell(40,5,$data['municipio'],'B',0,'L');

        $this->Cell(20,5,utf8_decode('Departamento '),0,0);
        $this->Cell(20,5,$data['departamento'],'B',0,'L');
        $this->Ln();

        $this->Cell(30,5,utf8_decode('Correo Electrónico :'),0,0);
        $this->Cell(60,5,utf8_decode($data['correo']),'B',0,'L');
        $this->Ln();

        $this->Cell(30,5,utf8_decode('Predio a Conectar '),0,0);
        $this->Cell(60,5,utf8_decode($data['direccion'] . ' ' . $data['barrio']),'B',0,'L');

        $this->Cell(15,5,utf8_decode('Municipio '),0,0);
        $this->Cell(50,5,utf8_decode($data['municipio'] .' - '. $data['departamento']),'B',0,'L');
        $this->Ln();
	}
	...
	function firmas_2(){
	    ...
	    $this->Cell(90, 30, '', 'B', 0, 'C');
	    $this->Ln();



	    $this->Cell(20,5,'NOMBRE:',0,0);
        $this->Cell(60,5,utf8_decode($data['nombre_suscriptor']),0,0,'L');
        $this->Ln();

        $this->Cell(20,5,'C.C:',0,0);
